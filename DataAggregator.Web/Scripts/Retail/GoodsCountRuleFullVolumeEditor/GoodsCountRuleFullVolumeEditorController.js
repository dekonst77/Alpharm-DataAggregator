angular
    ...
    .controller('GoodsCountRuleFullVolumeEditorController', ['$scope', '$http', '$uibModal', 'uiGridCustomService', 'formatConstants', '$translate', 'messageBoxService', GoodsCountRuleFullVolumeEditorController]);

function GoodsCountRuleFullVolumeEditorController($scope, $http, $uibModal, uiGridCustomService, formatConstants, $translate, messageBoxService) {
    ...
    $scope.countRuleFullVolumeEditorGrid.options.columnDefs = [
        ...
        { displayName: 'RETAIL.COUNT_RULE_EDITOR.SOURCE', field: 'FullGoodsDescription' },
        { displayName: 'RETAIL.COUNT_RULE_EDITOR.DESTINATION', field: 'DistributionFullGoodsDescription' },
    ...
    function fieldsAreValid() {
        ...
        if (!rule.FullGoodsDescription)
            return false;

        if (!rule.DistributionFullGoodsDescription && !$scope.rule.TopCountTo)
            return false;
    ...
    function loadRules() {
        ...
        $scope.loading = $http.get('/GoodsCountRuleFullVolumeEditor/LoadRules', { params: data })
    ...
    $scope.searchGoods = function(isDistribution) {
        var modalInstance = $uibModal.open({
            animation: true,
            templateUrl: 'Views/Retail/GoodsCountRuleFullVolumeEditor/_SearchGoodsView.html',
            controller: 'SearchGoodsFullVolumeController',
            size: 'full',
    ...
    $scope.deleteItem = function () {
        ...
        result.then(function () {
            ...
            $scope.loading = $http.post('/GoodsCountRuleFullVolumeEditor/DeleteRow', JSON.stringify({ id: id }))
                .then(function () {
    ...
    $scope.saveItem = function () {
        ...
        $scope.loading = $http.post('/GoodsCountRuleFullVolumeEditor/SaveRow', JSON.stringify({ model: $scope.rule }))
            .then(function (response) {
    ...
    $scope.$watch(
        function () { return $scope.rule.DistributionGoodsId; },
        function (newValue, oldValue) {
            if (newValue === oldValue)
                return;
    ...
    $scope.$watch(
        ...
        function (newValue, oldValue) {
            ...
            if ($scope.rule.DistributionGoodsId) {
                var rule = $scope.rule;
