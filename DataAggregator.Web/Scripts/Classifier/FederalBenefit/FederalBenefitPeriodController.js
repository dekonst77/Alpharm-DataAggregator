angular
    ...
    .controller('FederalBenefitPeriodController', ['$scope', '$http', '$uibModalInstance', '$uibModal', 'messageBoxService', 'uiGridCustomService', 'formatConstants', FederalBenefitPeriodController]);


function FederalBenefitPeriodController($scope, $http, $modalInstance, $uibModal, messageBoxService, uiGridCustomService, formatConstants) {

    //Задаем свойства грида
    $scope.gridOptions = uiGridCustomService.createOptions('FederalBenefit_PeriodGrid');
    ...
    $scope.Change = function () {
        ...
        $scope.periodLoading = $http({
            method: "POST",
            url: "/FederalBenefit/PeriodChange/",
            data: JSON.stringify({ period: $scope.Period })
        }).then(function (response) {
            var responseData = response.data;
    ...
    $scope.Remove = function () {
        $scope.periodLoading = $http({
            method: "POST",
            url: "/FederalBenefit/PeriodRemove/",
            data: JSON.stringify({ period: $scope.Period })
        }).then(function (response) {
            var responseData = response.data;
    ...
    $scope.CopyFederalBenefit = function () {
        var modalInstance = $uibModal.open({
            animation: true,
            templateUrl: 'Views/Classifier/FederalBenefit/_PeriodCopy.html',
            controller: 'FederalBenefitPeriodCopyController',
            size: 'giant',
            backdrop: 'static',
            resolve: {
                data: {
