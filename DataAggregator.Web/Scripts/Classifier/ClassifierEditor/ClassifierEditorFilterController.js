angular
    ...
    .controller('ClassifierEditorFilterController', ['$scope', '$http', '$uibModalInstance', 'classifierFilter', ClassifierEditorFilterController]);

function ClassifierEditorFilterController($scope, $http, $modalInstance, classifierFilter) {
    ...
    $scope.pressTypeaHeadEnter = function() {
        ...
        //Счетчик станет 2 - и для следующего enter сработает pressedEnter > 1
        pressedEnter++;
    };
    ...
    if ($scope.classifierFilter == null || $scope.classifierFilter.used == null)
        $scope.classifierFilter.used = 1;
