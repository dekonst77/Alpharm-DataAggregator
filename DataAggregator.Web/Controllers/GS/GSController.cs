...
GS_Brick*/
        ...
        public ActionResult GS_Clone(int GSId, DateTime currentperiod)
        {
            ...
            using (var _context = new GSContext(APP))
            {
                ...
                return GS_search(new FilterReestr() 
                { 
                    IDS = clone_GS.Id.ToString(), 
                    isCall = false, 
                    isCloseOFD = false,
...
*/
        ...
        public ActionResult History_ToFreeze(string Source_client)
        {
            ...
            {
                _context.Database.CommandTimeout = 0;
                _context.Database.ExecuteSqlCommand(" [adr].[History_coding_ToFreeze] @Source_client ,@user",
        ...
        public ActionResult History_UnFreeze(string Source_client)
        {
            ...
            {
                _context.Database.CommandTimeout = 0;
                _context.Database.ExecuteSqlCommand(" [adr].[History_coding_UnFreeze] @Source_client ,@user",
                    new System.Data.SqlClient.SqlParameter { ParameterName = "@Source_client", SqlDbType = System.Data.SqlDbType.NVarChar, Value = Source_client },
                    new System.Data.SqlClient.SqlParameter { ParameterName = "@user", SqlDbType = System.Data.SqlDbType.NVarChar, Value = User.Identity.GetUserId().ToString() }
                     );
            }
        ...
        public ActionResult SummsAnket_Save(ICollection<DataAggregator.Domain.Model.GS.SummsPeriod_Anket_SP> array, int count_period_edit)
        {
            try
            {
                using (var _context = new GSContext(APP))
                {
                    ...
                    foreach (var item in array)
                    {
                        var UPDs_3 = _context.GS_Period_Network.Where(w => w.NetworkName == item.NetworkName && w.Region == item.Region && w.Period == item.Period).FirstOrDefault();
                        if (UPDs_3 != null)
