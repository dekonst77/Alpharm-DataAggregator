...
{
    public class PriceCurrent_v2Controller : BaseController
    {

        private readonly OFDContext _context;


        public PriceCurrent_v2Controller()
        {
            _context = new OFDContext(APP);
        }

        ~PriceCurrent_v2Controller()
        {
            _context.Dispose();
        }
        ...
        public ActionResult Edit(PriceCurrentModel price)
        {
            try
            {
                ...
                var priceEtalon = _context.PriceCurrent_v2.First(p => p.ClassifierId == price.ClassifierId);
                ...
                var priceEtalonView = _context.PriceCurrentView_v2.First(p => p.ClassifierId == price.ClassifierId);
                var priceEtalonModel = PriceCurrentModel_v2.Create(priceEtalonView);
        ...
        public ActionResult RunCalcCurrentPrice()
        {
            try
            {
                _context.RunCalcCurrentPrice_v2();
                return null;
            }
        ...
        public ActionResult CurrentPriceCopyToEtalonPrice(DateTime date)
        {
            ...
            {
                var userGuid = new Guid(User.Identity.GetUserId());
                _context.CurrentPriceCopyToEtalonPrice_v2(date.Year, date.Month, userGuid);
                return null;
            }
            catch (ApplicationException e)
            {
        ...
        public ActionResult Load()
        {
            try
            {
                ...
                var priceView = _context.PriceCurrentView_v2.ToList();

                var priceetalonmodel = priceView.Select(PriceCurrentModel_v2.Create).ToList();
